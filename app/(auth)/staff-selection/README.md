...
| スタッフ選択エリア | StaffSelector | ◯ | チーム選択後 | - | グリッドレイアウト |
| ログインボタン | Button | - | スタッフ選択後 | 選択したスタッフでログイン | 青色スタイル、中央配置 |
...
| スタッフ選択 | スタッフを選択し、ログインボタンを表示 | - | 同一画面（ログインボタン表示） |
| ログインボタン | 選択したスタッフでログイン処理を実行 | `/api/v1/auth/staff/login` | ダッシュボード画面 (`/`) |
...
- **スタッフ選択**: 1列（モバイル）→ 2列（タブレット）→ 3列（デスクトップ）
