...
| 下書き保存       | 入力内容を下書きとして保存     | `POST /v1/handovers/draft` | 同一画面（成功メッセージ表示）  |
| 送信ボタン       | 申し送りを確定して送信         | `POST /v1/handovers`       | 申し送り一覧画面 (`/handovers`) |
...
| メソッド | エンドポイント         | 説明             |
| -------- | ---------------------- | ---------------- |
| POST     | `/v1/handovers`        | 申し送り新規作成 |
| POST     | `/v1/handovers/draft`  | 下書き保存       |
| PUT      | `/v1/handovers/{id}`   | 申し送り更新     |
| GET      | `/v1/handovers/drafts` | 下書き一覧取得   |
