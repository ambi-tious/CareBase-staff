...
| 下書き保存       | 入力内容を下書きとして保存     | `PUT /v1/contact-schedule/{id}/draft` | 同一画面（成功メッセージ表示）                |
| 更新ボタン       | 連絡・予定を確定して更新       | `PUT /v1/contact-schedule/{id}`       | 連絡・予定詳細画面 (`/contact-schedule/{id}`) |
...
| メソッド | エンドポイント                    | 説明               |
| -------- | --------------------------------- | ------------------ |
| GET      | `/v1/contact-schedule/{id}`       | 連絡・予定詳細取得 |
| PUT      | `/v1/contact-schedule/{id}`       | 連絡・予定更新     |
| PUT      | `/v1/contact-schedule/{id}/draft` | 下書き保存         |
