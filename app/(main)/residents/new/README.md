...
| 所属フロア・グループ | FormSelect     | ◯    | 常時         | localStorage自動取得 | 常時無効化、ログイン職員のグループを自動設定      |
| 所属ユニット・チーム | FormSelect     | ◯    | 常時         | localStorage自動取得 | 常時無効化、ログイン職員のチームを自動設定        |
...
| 認定有効期間開始日   | FormField      | ◯    | 常時         | 空文字               | 日付入力                                          |
| 認定有効期間終了日   | FormField      | ◯    | 常時         | 空文字               | 日付入力                                          |
...
- **フィールド制御**: 自動設定されたフィールドは常時 `disabled={true}` で編集不可
...
| 認定有効期間開始日 | input    | 必須チェック               | 認定有効期間開始日は必須です                   |
| 認定有効期間終了日 | input    | 必須チェック               | 認定有効期間終了日は必須です                   |
...
- **所属フロア・グループ**: `disabled={true}` で常時編集不可
- **所属ユニット・チーム**: `disabled={true}` で常時編集不可
- **視覚的表示**: グレーアウト表示で自動設定であることを明示
