...
### ResidentBasicInfo（利用者基本情報）

```typescript
interface ResidentBasicInfo {
  name: string;           // 氏名（必須）
  furigana: string;       // フリガナ（必須、カタカナのみ）
  dob: string;            // 生年月日（必須、YYYY-MM-DD形式）
  sex: '男' | '女' | 'その他' | '';  // 性別（必須）
  careLevel: string;      // 要介護度（必須）
  floorGroup: string;     // 所属フロア・グループ（任意）
  unitTeam: string;       // 所属ユニット・チーム（任意）
  roomInfo: string;       // 部屋情報（任意）
  address: string;        // 住所（必須）
  admissionDate: string;  // 入所日（必須、YYYY-MM-DD形式）
}
```

### FormValidationErrors（フォームバリデーションエラー）

```typescript
type FormValidationErrors = Partial<Record<keyof ResidentBasicInfo, string>>;
```
