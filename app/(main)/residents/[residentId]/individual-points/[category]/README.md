...
利用者の個別ポイントカテゴリごとの詳細情報を表示・編集・削除する機能を提供します。  
...
| 保存ボタン           | Button         | -    | 編集モード時     | 保存                             | 青色、Save アイコン            |
| 詳細内容表示エリア   | Div            | -    | 閲覧モード時     | -                                | 改行保持、最小高さ384px        |
| 詳細内容入力エリア   | Textarea       | -    | 編集モード時     | -                                | 高さ384px、リサイズ不可        |
...
| キャンセルボタン | 編集をキャンセルして閲覧モードに | -                                                            | 同一画面（閲覧モード）             |
| 保存ボタン       | 詳細内容を保存                   | `/api/v1/residents/{id}/individual-points/{category}`        | 同一画面（閲覧モード）             |
...
- **保存ボタン**: `bg-carebase-blue hover:bg-carebase-blue-dark`
