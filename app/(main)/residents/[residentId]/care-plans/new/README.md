...
| 下書き保存ボタン   | ケアプラン情報を下書きとして保存       | `POST /v1/care-plans` | 同一画面（成功メッセージ表示後継続編集可能）                       |
| 作成ボタン         | ケアプラン情報を保存して詳細画面に遷移 | `POST /v1/care-plans` | ケアプラン詳細画面 (`/residents/[residentId]/care-plans/[planId]`) |
...
| メソッド | エンドポイント               | 説明               |
| -------- | ---------------------------- | ------------------ |
| POST     | `/v1/care-plans`             | ケアプラン新規作成 |
| GET      | `/v1/residents/{residentId}` | 利用者情報取得     |
