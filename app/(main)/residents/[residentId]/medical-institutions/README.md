...
| 登録処理 | 新しいかかりつけ医療機関を登録 | `/api/v1/residents/{id}/medical-institutions` | 同一画面（モーダル閉じる） |
| 編集処理 | 既存のかかりつけ医療機関を更新 | `/api/v1/residents/{id}/medical-institutions/{institutionId}` | 同一画面（モーダル閉じる） |
...
| 医療機関名入力 | Input | ◯ | - | プレースホルダー: 松本内科クリニック |
| 医師名入力 | Input | ◯ | - | プレースホルダー: 松本医師 |
| 電話番号入力 | Input | ◯ | - | プレースホルダー: 078-000-0000 |
| FAX入力 | Input | - | - | プレースホルダー: 078-0000-0000 |
| 住所入力 | Input | ◯ | - | プレースホルダー: 兵庫県神戸市西区新川1名ヶ原4-5-1 |
...
| --- | --- | --- | --- |
| 医療機関名入力 | blur | 必須入力チェック | 医療機関名は必須です |
| 医療機関名入力 | blur | 文字数チェック（100文字以内） | 医療機関名は100文字以内で入力してください |
| 医師名入力 | blur | 必須入力チェック | 医師名は必須です |
| 医師名入力 | blur | 文字数チェック（50文字以内） | 医師名は50文字以内で入力してください |
| 電話番号入力 | blur | 必須入力チェック | 電話番号は必須です |
| 電話番号入力 | blur | 形式チェック | 有効な電話番号を入力してください |
| FAX入力 | blur | 形式チェック | 有効なFAX番号を入力してください |
...
- **電話番号・FAX**: 数字、ハイフン、プラス、括弧、スペースのみ許可
- **医療機関名**: 1文字以上100文字以内
- **医師名**: 1文字以上50文字以内
- **住所**: 1文字以上200文字以内
- **備考**: 任意入力、制限なし
