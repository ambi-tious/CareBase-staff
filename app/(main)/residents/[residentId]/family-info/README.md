...
| 登録処理   | 新しいご家族情報を登録       | `/v1/residents/{id}/contacts`             | 同一画面（モーダル閉じる） |
| 編集処理   | 既存のご家族情報を更新       | `/v1/residents/{id}/contacts/{contactId}` | 同一画面（モーダル閉じる） |
| 削除処理   | 既存のご家族情報を削除       | `/v1/residents/{id}/contacts/{contactId}` | 同一画面（モーダル閉じる） |
