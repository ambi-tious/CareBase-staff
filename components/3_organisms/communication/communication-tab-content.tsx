...
import {
  getCommunicationRecordsByResident,
  getCommunicationThreadsByResident,
} from '@/mocks/communication-data';
...
export const CommunicationTabContent = forwardRef<
  CommunicationTabContentRef,
      ...
      setThreads((prev) =>
        prev.map((thread) =>
          thread.id === updatedRecord.threadId
            ? {
                ...thread,
                records: thread.records.map((r) => (r.id === updatedRecord.id ? updatedRecord : r)),
