...
function TimeBaseView({ residents }: { residents: Resident[] }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (currentTimeElement && scrollContainerRef.current) {
        ...
        const offset =
          elementTop - containerTop - scrollContainerRef.current.clientHeight / 2 + currentTimeElement.clientHeight / 2
  ...
  function EventCell({ events, time }: { events: CareEvent[]; time: string }) {
    ...
    return (
      <div className={`h-10 border-b border-gray-200 p-1 flex flex-col flex-wrap items-start justify-start gap-0.5`}>
            ...
            >
              ...
              >
                <div className="relative w-8 h-8 rounded-full overflow-hidden mb-1">
                  ...
                  <Image
                    src={resident.avatarUrl || "/placeholder.svg?height=32&width=32&query=default+avatar"}
                    alt={resident.name}
                    fill
